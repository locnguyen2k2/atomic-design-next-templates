  ...
  return (
    <div className="data-table-wrapper">
      ...
      <div className="table-toolbar flex items-center gap-4 p-4 border-b border-border">
        ...
        <span className="toolbar-results text-sm text-text-muted">
          {pagination?.totalItems ?? data.length} records
        </span>
                  ...
                  {columns.map((col) => (
                    <td key={String(col.key)} className="px-4 py-3">
                      {col.render ? col.render(row) : (
                        <span className="text-sm">{String(row[col.key as keyof T] ?? '—')}</span>
                      )}
                    </td>
                  ))}
