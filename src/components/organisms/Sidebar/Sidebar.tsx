...
export function Sidebar({ currentOrg, organizations, onOrgSwitch, collapsed, onToggle, mobileOpen, onMobileClose, user }: SidebarProps) {
  const [orgDropdownOpen, setOrgDropdownOpen] = useState(false);
  const pathname = usePathname();

  return (
    <>
      {mobileOpen && <div className="fixed inset-0 bg-black/60 z-40 lg:hidden transition-opacity duration-300" onClick={onMobileClose} />}

      <aside className={cn("sidebar fixed left-0 top-0 h-full bg-bg-elevated border-r border-border z-50", "transition-all duration-300 ease-in-out overflow-x-hidden", collapsed ? "lg:w-[68px]" : "lg:w-[260px]", mobileOpen ? "translate-x-0 w-[260px]" : "-translate-x-full lg:translate-x-0")}>
        {/* Logo */}
        <div className="sidebar-header flex items-center gap-3 p-4 border-b border-border">
          ...
          <button onClick={onToggle} className="ml-auto p-2 rounded-lg hover:bg-bg-surface transition-all duration-200 hover:scale-105">
        ...
        <div className="org-switcher relative">
          <button onClick={() => setOrgDropdownOpen(!orgDropdownOpen)} className="org-switcher-inner flex items-center gap-2.5 w-full p-2 rounded-lg hover:bg-bg-surface transition-colors duration-200">
            ...
            {!collapsed && (
              <>
                ...
                <Icon name="chevron-down" className={cn("org-chevron text-xs text-text-muted transition-transform duration-200", orgDropdownOpen && "rotate-180")} />
              </>
            )}
          </button>
          ...
          {orgDropdownOpen && !collapsed && (
            <div className="org-dropdown m-3 absolute top-[calc(100%+6px)] left-0 right-0 bg-bg-surface rounded-lg border border-border shadow-lg overflow-hidden z-[60] min-w-[200px]">
              <div className="org-dropdown-header text-xs font-semibold uppercase tracking-wide text-text-muted py-2.5 px-3.5 pb-1.5">Switch Organization</div>
              <div className="org-dropdown-list">
                {organizations.map((org) => (
                  <button
                    ...
                    className={cn("org-dropdown-item flex items-center gap-2.5 w-full p-2.5 hover:bg-bg-elevated transition-colors duration-200", org.id === currentOrg.id && "bg-primary-dim")}
                  >
                    ...
                    {org.id === currentOrg.id && <Icon name="check" className="ml-auto text-primary text-xs" />}
                  </button>
                ))}
              </div>
          ...
          {collapsed &&
            navItems.map((item) => (
              <Link
                key={item.id}
                href={item.href}
                className={cn("nav-item flex items-center justify-center px-3 py-2.5 rounded-lg mb-1", "text-text-secondary hover:text-text-primary hover:bg-bg-surface", "transition-colors duration-200", pathname === item.href && "active bg-primary-dim text-primary")}
              >
                <Icon name={item.icon} />
              </Link>
            ))}
        ...
        <div className="sidebar-footer p-3 border-t border-border">
          <Link href="/profile" className="flex items-center gap-3 p-2 rounded-lg hover:bg-bg-surface">
